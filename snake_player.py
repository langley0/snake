...
def play(game, screen, params):
    ...
    while True: 
        ...
        game.draw(screen)
        # check player input
        action = get_next_action(last_state, 0)
        last_state = build_state(game)
        game.player_direction = action
        reward = game.update()
        if reward <= -1:
            ...
            score += reward
            loop += 1
        time.sleep(time_per_frame - (time.time() - _t))
        
    return score, loop

if __name__ == '__main__':
    model_file = './save/snake-model9-1164100'
    ...
    with sess.as_default():
        ...
        for ngame in range(MAX_GAME): 
            ...
            score, loop = play(game, screen, params)
